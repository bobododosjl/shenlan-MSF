...
void ErrorStateKalmanFilter::UpdateErrorEstimation(
  ...
  // B.block<6, 6>(9, 6) = Eigen::Matrix<double, 6, 6>::Zero();
...
void ErrorStateKalmanFilter::CorrectErrorEstimationPose(
  ...
  Eigen::Matrix3d R_nn_obs = T_nb.block<3, 3>(0, 0); // fix this

  Eigen::Vector3d P_nn_mea = pose_.block<3, 1>(0, 3);
  Eigen::Matrix3d R_nn_mea = pose_.block<3, 3>(0, 0);
  YPose_.block<3, 1>(0, 0) = P_nn_mea - P_nn_obs;
  YPose_.block<3, 1>(3, 0) = Sophus::SO3d::vee(R_nn_obs.transpose() * R_nn_mea - Eigen::Matrix3d::Identity());
