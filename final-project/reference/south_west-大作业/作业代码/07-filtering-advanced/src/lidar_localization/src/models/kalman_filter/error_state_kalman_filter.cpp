...
ErrorStateKalmanFilter::ErrorStateKalmanFilter(const YAML::Node& node) {
    ...
    RPoseVelCons_.block<3, 3>(0, 0) = COV.MEASUREMENT.POSE.POSI*Eigen::Matrix3d::Identity();
    RPoseVelCons_.block<2, 2>(3, 3) = COV.MEASUREMENT.VEL*Eigen::Matrix2d::Identity();
    RPoseVelCons_.block<3, 3>(5, 5) = COV.MEASUREMENT.POSE.ORI*Eigen::Matrix3d::Identity();
...
void ErrorStateKalmanFilter::SetProcessEquation(const Eigen::Matrix3d &C_nb,
  ...
  F_.block<3, 3>(6, 12) = -Eigen::Matrix3d::Identity();
...
void ErrorStateKalmanFilter::CorrectErrorEstimationPosiVel(
    ...
    GPosiVel_.block<3, 3>(3, INDEX_ERROR_ORI) = Sophus::SO3d::hat(v_b).matrix();
...
void ErrorStateKalmanFilter::CorrectErrorEstimationPoseVelCons(
    ...
    if ( !IsTurning(w_b) && MOTION_CONSTRAINT.ACTIVATED ) {
        ...
        Eigen::Vector3d P_nn_obs = pose_.block<3, 1>(0,3) - T_nb.block<3, 1>(0,3);
        Eigen::Matrix3d C_nn_obs = T_nb.block<3, 3>(0,0).transpose() * pose_.block<3, 3>(0,0);
        Eigen::Vector3d v_bb_obs = pose_.block<3, 3>(0,0).transpose()*vel_;

        Eigen::Matrix<double, 8, 1> YPoseVelCons = Eigen::Matrix<double, 8, 1>::Zero();
        YPoseVelCons.block<3, 1>(0, 0) = P_nn_obs;
        YPoseVelCons.block<2, 1>(3, 0) = v_bb_obs.block<2, 1>(1, 0);
        YPoseVelCons.block<3, 1>(5, 0) = Sophus::SO3d::vee(C_nn_obs - Eigen::Matrix3d::Identity());

        Y = YPoseVelCons;

        // set measurement equation, with motion constraint:
        Eigen::Matrix<double, 8, 15> GPoseVelCons = Eigen::Matrix<double, 8, 15>::Zero();
        GPoseVelCons.block<3, 3>(0, 0) = Eigen::Matrix3d::Identity();
        GPoseVelCons.block<2, 2>(3, 3) = pose_.block<3, 3>(0,0).transpose().block<2, 2>(1, 1);
        GPoseVelCons.block<3, 3>(5, 6) = Eigen::Matrix3d::Identity();

        G = GPoseVelCons;

        Eigen::Matrix<double, 8, 8> CPoseVelCons = Eigen::Matrix<double, 8, 8>::Identity();
        // set Kalman gain, with motion constraint:
        MatrixRPoseVelCons RCons = GPoseVelCons*P_*GPoseVelCons.transpose() + CPoseVelCons*RPoseVelCons_*CPoseVelCons.transpose();
        K = P_*GPoseVelCons.transpose()*RCons.inverse();
    } else {
        // set measurement:
        Eigen::Vector3d P_nn_obs = pose_.block<3, 1>(0,3) - T_nb.block<3, 1>(0,3);
        Eigen::Matrix3d C_nn_obs = T_nb.block<3, 3>(0,0).transpose() * pose_.block<3, 3>(0,0);
