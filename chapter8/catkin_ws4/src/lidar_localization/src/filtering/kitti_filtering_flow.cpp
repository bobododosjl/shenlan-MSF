...
bool KITTIFilteringFlow::SaveVelocity(void) {
 ...
 if(!FileManager::CreateFile(robot_velocity_ofs, WORK_SPACE_PATH + "/slam_data/trajectory/robot_velocity.txt")){
      return false;
  }
  ...
  for (size_t i = 0;i <trajectory.N; ++i){
      ...
      const Eigen::Vector3f robot_vel = fused_pose.block<3, 3>(0, 0).transpose() * fused_vel;
      ///save robot vel into the output
      robot_velocity_ofs << time << " " << robot_vel.x() << " " << robot_vel.y() << " " << robot_vel.z() << std::endl;
  }
    return true;
}
